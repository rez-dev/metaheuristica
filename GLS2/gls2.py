...
def actualizar_penalizaciones(solucion, penalizaciones):
    for i in range(len(solucion)-1):
        penalizaciones[solucion[i]][solucion[i + 1]] += 1
...
# DISTANCIA AUMENTADA
def distancia_aumentada(matriz_distancias, penalizaciones, landa):
    matriz_distancias_aumentada = [[0 for _ in range(len(matriz_distancias))] for _ in range(len(matriz_distancias))] # matriz de 0´s
    for i in range(len(matriz_distancias)):
        for j in range(len(matriz_distancias[i])):
            matriz_distancias_aumentada[i][j] = matriz_distancias[i][j] + landa * penalizaciones[i][j]
    return matriz_distancias_aumentada

# GUIDED LOCAL SEARCH
def guided_local_search(landa, max_iteraciones,matriz_distancias):
    # Generar solucion inicial
    ...
    matriz_distancias_aumentada = distancia_aumentada(matriz_distancias, penalizaciones, landa) # D'ij = Dij + landa * Pij

    # Iterar hasta que se cumpla el criterio de parada
    iteracion = 0
    while iteracion < max_iteraciones:
        # Busqueda local
        nueva_solucion = local_search(solucion_actual, lambda x: calcular_costo(x, matriz_distancias_aumentada))
        nuevo_costo = calcular_costo(solucion_actual, matriz_distancias_aumentada)
        # Actualizar penalizaciones
        penalizaciones = actualizar_penalizaciones(nueva_solucion, penalizaciones)
        # Actualizar matriz de distancias aumentadas
        matriz_distancias_aumentada = distancia_aumentada(matriz_distancias, penalizaciones, landa)
