...
def guided_local_search(tsp, work, time_limit, max_call_count,penalty_ratio):
    ...
    cnt = 0
    while cur_time - start_time < time_limit and cur_work.call_count < max_call_count:
        # print(work.call_count)
        # if work.call_count >= max_call_count:
        #     break        
        best_obj = work.obj
        ...
        two_opt_search(tsp, work, cur_work,max_call_count)
        # update penalty
...
def two_opt_search(tsp, work, cur_work, max_call_count):
    ...
    restart = True
    while restart:
        if work.call_count >= max_call_count or cur_work.call_count >= max_call_count:
            print("MAXIMO ALCANZADO1 " + str(work.call_count) + " " + str(cur_work.call_count))
            restart = False
            break
        restart = False
        nbhd = ((u,v)
        ...
        for u,v in nbhd:
            ...
            if work.call_count > max_call_count or cur_work.call_count > max_call_count:
                print("MAXIMO ALCANZADO2 " + str(work.call_count) + " " + str(cur_work.call_count))
                restart = False
                break
            ...
            if delta < -NUM_EPSILON:
                ...
                restart = True
                if work.call_count >= max_call_count or cur_work.call_count >= max_call_count:
                    print("MAXIMO ALCANZADO3 " + str(work.call_count) + " " + str(cur_work.call_count))
                    restart = False
                break
    return improved
