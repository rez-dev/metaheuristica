...
def ruta_nn(tsp, solucion):
    print('\n[ruta mejorada]')
    ruta = nearest_neighbor(tsp)  # Crea una lista de nodos en orden secuencial
    # Establece la ruta aleatoria como la nueva ruta de trabajo
    solucion.ruta = ruta
    solucion.definir_pos()
    solucion.recorrido_total = solucion.largo(tsp)
...
def guided_local_search(tsp, max_call_count,grado_penalizacion, cantidad_no_mejoras):
    ...
    while contador < max_call_count:
        ...
        # busqueda local
        contador = dos_opt(tsp, solucion, solucion_actual,max_call_count, temp, cantidad_no_mejoras)
        ...
        datos_mejorados.append(solucion.recorrido_total)
    ...
    return datos_convergencia, solucion, datos_mejorados
...
#     mejora = False
#     reiniciar = True
#     while reiniciar and nuevo_contador < max_call_count:
#         reiniciar = False
...
def dos_opt(tsp, solucion, solucion_actual, max_call_count, contador, n):
    nuevo_contador = contador
    numero_mejoras = 0  # Contador para el número de mejoras realizadas
    mejora = False
    reiniciar = True
    while reiniciar and nuevo_contador < max_call_count:
        reiniciar = False
        ...
        for u, v in vecindario:
            ...
            if solucion_actual.recorrido_total + delta < solucion.recorrido_total - NUM_EPSILON:
                ...
                nuevo_contador += 1
                numero_mejoras += 1  # Incrementar el contador de mejoras
                if numero_mejoras >= n:  # Verificar el límite de mejoras
                    return nuevo_contador
...
def main():
    # parametros
    grado_penalizacion = 3.903954688575139
    max_call_count = 10000
    cantidad_vecinos = 5
    cantidad_no_mejoras = 49
    ...
    # resolver TSP
    # solucion = Solucion(tsp,grado_penalizacion)
    datos_convergencia, solucion, datos_mejorados = guided_local_search(tsp, max_call_count,grado_penalizacion, cantidad_no_mejoras)
