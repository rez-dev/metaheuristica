...
# --------------------------------------------------------------------
#   Guided Local Search for TSP with Fast Local Search
#
...
def guided_local_search(tsp, work, time_limit):
    ...
    def update_penalty(tsp,work):
        ...
        for v in work.tour:
            (work.active)[v] = False
        (work.active)[v] = (work.active)[next_v] = True
        for u in (tsp.neighbor)[v]:
            (work.active)[u] = True
        for u in (tsp.neighbor)[next_v]:
            (work.active)[u] = True
    ...
    cur_work.active = [True for _ in range(len(cur_work.tour))]
...
def local_search(tsp, work, cur_work):
    ...
    while restart:
        ...
        for u in cur_work.tour:
            if (cur_work.active)[u]:
                # 2-opt neighborhood search
                if two_opt_search(tsp, work, cur_work, u):
                    restart = True
                    break
                # # Or-opt neighborhood search
                # if or_opt_search(tsp, work, cur_work, u):
                #     restart = True
                #     break
                # # 3-opt neighborhood search
                # if three_opt_search(tsp, work, cur_work, u):
                #     restart = True
                #     break
            # inactivate node u
            (cur_work.active)[u] = False
...
# --------------------------------------------------------------------
def two_opt_search(tsp, work, cur_work, u):
    ...
    for v in (tsp.neighbor)[u]:
        ...
        if delta < -NUM_EPSILON:
            # activate nodes
            (cur_work.active)[u] = (cur_work.active)[cur_work.next(u)] = True
            (cur_work.active)[v] = (cur_work.active)[cur_work.next(v)] = True
            # change current tour
...
#   return: [True] improved
# --------------------------------------------------------------------
def or_opt_search(tsp, work, cur_work, u, size = OR_OPT_SIZE):
...
#   tsp(I): TSP data
#   work(I/O): working data
#   cur_work(/O): current working data
#   u(I): pivot node
#   return: [True] improved
# --------------------------------------------------------------------
def three_opt_search(tsp, work, cur_work, u):
    ...
    for v,w in nbhd:
        ...
        if delta < -NUM_EPSILON:
            # activate nodes
            activate_node(cur_work, u, v, w, oper)
            # change current tour
